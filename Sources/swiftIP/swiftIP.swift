...
public struct IPFormatStyle: IPFormatter, Equatable {
    ...
    public func string(from ip: IP) -> String {
        switch style {
        case .dotDecimal:
            return ip.bytes.map { String($0) }.joined(separator: ".")
        case .dotHexadecimal:
            return ip.bytes.map { String(format: "0x%02X", $0) }.joined(separator: ".")
        case .dotOctal:
            return ip.bytes.map { String(format: "%04O", $0) }.joined(separator: ".")
        case .decimal:
            return String(ip.bytes.reduce(0) { ($0 << 8) + Int($1) })
        case .hexadecimal:
            return "0x" + String(ip.bytes.reduce(0) { ($0 << 8) + Int($1) }, radix: 16, uppercase: true)
        case .octal:
            return "0" + String(ip.bytes.reduce(0) { ($0 << 8) + Int($1) }, radix: 8, uppercase: true)
        }
    }
}
...
public struct IPv4: IP {
    ...
    init?(from: String) {
        ...
        let bytes = segment.compactMap { UInt8($0) }
        self.init(from: bytes)
    }

    init?(from: [UInt8]) {
        if from.count != lenght {
            return nil
        }
        bytes = from
    }
