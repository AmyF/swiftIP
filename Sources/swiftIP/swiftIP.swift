...
public protocol IP {
    ...
    func masking(by mask: IPMask) -> IP?
}

public extension IP {
    public func isEqual(to: IP) -> Bool {
        switch (self.isIPv4, to.isIPv4) {
        case (true, true):
            return self.bytes == to.bytes
        case (true, false):
            return [UInt8](to.bytes[...12]) == _v4InV6Prefix && self.bytes == [UInt8](to.bytes[12...])
        case (false, true):
            return [UInt8](self.bytes[...12]) == _v4InV6Prefix && to.bytes == [UInt8](self.bytes[12...])
        case (false, false):
            return self.bytes == to.bytes
        }
    }
